...
if (!s.includes(oldFn)) {
  console.error('getTranslation block not found');
  process.exit(1);
}
s = s.replace(oldFn, newFn);

const oldDiscover = `      exploreDining: 'Explore dining options',
...
const newDiscover = `      exploreDining: 'Explore dining options',
      exploreHotels: 'Browse hotels and stays',
...
if (!s.includes(oldDiscover)) {
  console.error('EN placeDiscover/dining/hotel block not found (file changed?)');
  process.exit(1);
}
s = s.replace(oldDiscover, newDiscover);

fs.writeFileSync(file, s, 'utf8');
console.log('Patched translations.js (en fallback + dining/hotels/discover copy)');
