...
export default function DiningMealListingCard({
  ...
  const offerTeaser = offerHighlight ? diningOfferTeaser(place) : "";
  ...
  return (
    <article
      className={`hg-stay-card hg-stay-card--dining${sponsoredBadge ? " hg-stay-card--partner" : ""}`}
    >
      ...
      <div
        className={`hg-dining-card__footer${
          quickRow.length > 0 ? " hg-dining-card__footer--has-quick" : ""
        }`}
      >
        {quickRow.length > 0 ? (
...
Path("client/src/components/DiningMealListingCard.jsx").write_text(content, encoding="utf-8")
print("ok")
