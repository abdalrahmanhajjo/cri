/**
 * Icon semantics + Flaticon guidance.
 *
 * Animated art: we ship lightweight Lottie placeholders per “variant” family. To use
 * assets from https://www.flaticon.com/animated-icons (most downloaded or any pack),
 * download the **Lottie JSON** for each symbol and save as:
 *   client/src/assets/flaticon-overrides/{materialSymbolName}.json
 * (same name as the `name` prop on `<Icon />`). Follow Flaticon’s license and
 * attribution rules for your plan (free vs premium).
 *
 * Each entry explains what the icon communicates in this app and what to pick on Flaticon.
 *
...
export function getIconDoc(name) {
  ...
  const human = String(name).replaceAll('_', ' ');
  return {
    variant,
    ...
    flaticonAnimated: `On Flaticon animated icons, search for “${human}” (or closest match) and export Lottie JSON as ${name}.json into src/assets/flaticon-overrides/.`,
  };
}
