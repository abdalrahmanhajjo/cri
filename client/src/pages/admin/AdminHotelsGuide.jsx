...
export default function AdminHotelsGuide() {
  const [full, setFull] = useState(() => mergeWithSiteSettingsDefaults({}));
  const [hotels, setHotels] = useState(() => mergeHotelsGuide({}));
  ...
  useEffect(() => {
    ...
    api.admin.siteSettings
      ...
      .then((r) => {
        ...
        setFull(merged);
        setHotels(mergeHotelsGuide(server.hotelsGuide));
      })
      .catch(() => {
        if (cancelled) return;
        const merged = mergeWithSiteSettingsDefaults({});
        setFull(merged);
        setHotels(merged.hotelsGuide);
      })
      .finally(() => {
  ...
  const stayCategoryIds = useMemo(() => {
    const ways = getCategoriesForWay('stay', categories);
  ...
  const featuredOffStayWay = useMemo(() => {
    const ids = hotels.featuredPlaceIds || [];
    return ids.filter((id) => {
      const p = placeById.get(String(id));
      if (!p) return false;
      return !stayCategoryIds.has(String(p.categoryId ?? p.category_id));
    });
  }, [hotels.featuredPlaceIds, placeById, stayCategoryIds]);
  ...
  const addFeatured = (place) => {
    ...
    const hidden = new Set((hotels.hiddenPlaceIds || []).map(String));
    if (hidden.has(id)) return;
    if ((hotels.featuredPlaceIds || []).some((x) => String(x) === id)) return;
    setHotels((d) => ({ ...d, featuredPlaceIds: [...(d.featuredPlaceIds || []), id] }));
    setFeatQ('');
    setFeatResults([]);
  };
  ...
  const addHidden = (place) => {
    const id = String(place.id);
    if ((hotels.hiddenPlaceIds || []).some((x) => String(x) === id)) return;
    setHotels((d) => ({
      ...d,
      hiddenPlaceIds: [...(d.hiddenPlaceIds || []), id],
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { ...full, hotelsGuide: hotels };
      ...
      setFull(merged);
      setHotels(mergeHotelsGuide(merged.hotelsGuide));
  ...
  const loc = localeTab;
  const h = hotels.hero?.[loc] || {};
  const sl = hotels.sectionLabels?.[loc] || {};

  if (loading) {
    return (
      <div className="admin-page-content">
        <p className="admin-form-hint">Loading hotels guide settings…</p>
      </div>
    );
  }
    ...
    <div className="admin-page-content">
      <div className="admin-page-header">
        <div className="admin-page-title-wrap">
          <h1 className="admin-page-title">Hotels guide</h1>
          <p className="admin-subtitle">
            Controls the public <Link to={HOTELS_PATH}>/hotels</Link> page: hero copy, imagery, curated order, exclusions, and section
            titles. Listings use the same <strong>stay</strong> theme as the home map — set categories in{' '}
            <Link to="../categories">Categories</Link>. Sponsored cards use surface <strong>Hotels</strong> on{' '}
            <Link to="../sponsored-places">Sponsored places</Link> (subject to Settings → feature toggles).
          </p>
        </div>
        <Link to={HOTELS_PATH} className="admin-btn admin-btn--secondary" target="_blank" rel="noreferrer">
          Open public page
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit} className="adg-form">
        ...
        <div className="admin-card" style={{ gridColumn: 'span 12' }}>
          ...
          <div className="admin-card-body">
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
              <input
                type="checkbox"
                checked={hotels.enabled !== false}
                onChange={(e) => setHotels((d) => ({ ...d, enabled: e.target.checked }))}
              />
        ...
        <div className="admin-card" style={{ gridColumn: 'span 12' }}>
          ...
          <div className="admin-card-body">
            <div className="admin-form-group">
              <label htmlFor="ahg-hero-img">Image URL</label>
              <input
                id="ahg-hero-img"
                type="url"
                value={hotels.heroImageUrl || ''}
                onChange={(e) => setHotels((d) => ({ ...d, heroImageUrl: e.target.value }))}
                placeholder="https://…"
              />
        ...
        <div className="admin-card" style={{ gridColumn: 'span 12' }}>
          ...
          <div className="admin-card-body">
            ...
            <div className="admin-form-row">
              <div className="admin-form-group">
                <label htmlFor={`ahg-kicker-${loc}`}>Kicker / eyebrow</label>
                <input
                  id={`ahg-kicker-${loc}`}
                  value={h.kicker || ''}
                  onChange={(e) => patchHero(loc, 'kicker', e.target.value)}
                  placeholder="Empty = app translation default (hotelGuide)"
                />
              </div>
              <div className="admin-form-group">
                <label htmlFor={`ahg-title-${loc}`}>Title</label>
                <input
                  id={`ahg-title-${loc}`}
                  value={h.title || ''}
            ...
            <div className="admin-form-group">
              <label htmlFor={`ahg-sub-${loc}`}>Subtitle</label>
              <textarea
                id={`ahg-sub-${loc}`}
                rows={3}
                value={h.subtitle || ''}
            ...
            <div className="admin-form-row admin-form-row--3">
              <div className="admin-form-group">
                <label htmlFor={`ahg-sl-sponsored-${loc}`}>Sponsored rail title</label>
                <input
                  id={`ahg-sl-sponsored-${loc}`}
                  value={sl.sponsoredKicker || ''}
                  onChange={(e) => patchSectionLabels(loc, 'sponsoredKicker', e.target.value)}
                />
              </div>
              <div className="admin-form-group">
                <label htmlFor={`ahg-sl-top-${loc}`}>Top picks title</label>
                <input
                  id={`ahg-sl-top-${loc}`}
              ...
              <div className="admin-form-group">
                <label htmlFor={`ahg-sl-main-${loc}`}>Main collection (screen-reader)</label>
                <input
                  id={`ahg-sl-main-${loc}`}
        ...
        <div className="admin-card" style={{ gridColumn: 'span 12' }}>
          ...
          <div className="admin-card-body">
            {featuredOffStayWay.length ? (
              <p className="admin-form-hint" style={{ color: '#b45309', marginTop: 0 }}>
                Some featured IDs are not in the stay-way category set — they will not appear on /hotels until their category matches:{' '}
                {featuredOffStayWay.join(', ')}
              </p>
            ) : null}
            <ol className="adg-ordered-list">
              {(hotels.featuredPlaceIds || []).map((id, index) => (
                <li key={String(id)} className="adg-ordered-item">
                  ...
                  <div className="adg-ordered-actions">
                    ...
                    <button
                      ...
                      className="admin-btn admin-btn--secondary admin-btn--sm"
                      onClick={() => moveFeatured(index, 1)}
                      disabled={index >= (hotels.featuredPlaceIds || []).length - 1}
                    >
                      Down
                    </button>
                    <button
            ...
            <div className="admin-form-group" style={{ marginTop: '1rem' }}>
              <label htmlFor="ahg-feat-search">Add place (search)</label>
              <input
                id="ahg-feat-search"
                value={featQ}
        ...
        <div className="admin-card" style={{ gridColumn: 'span 12' }}>
          <div className="admin-card-header">
            <h2 className="admin-card-title">Hidden from hotels</h2>
          </div>
          <div className="admin-card-body">
            <p className="admin-form-hint" style={{ marginTop: 0 }}>
              Excluded even when the place category is in the stay theme (e.g. duplicate listings).
            </p>
            <ul className="adg-chip-list">
              {(hotels.hiddenPlaceIds || []).map((id) => (
            ...
            <div className="admin-form-group">
              <label htmlFor="ahg-hid-search">Add to hidden</label>
              <input
                id="ahg-hid-search"
                value={hidQ}
        ...
        <div className="adg-form-actions">
          <button type="submit" className="admin-btn admin-btn--primary" disabled={saving}>
            {saving ? 'Saving…' : 'Save hotels guide'}
          </button>
          <Link to={HOTELS_PATH} className="admin-btn admin-btn--secondary" target="_blank" rel="noreferrer">
            Preview /hotels
          </Link>
