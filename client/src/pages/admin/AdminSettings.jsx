            ...
            {activeTab === 'features' && (
              <div className="admin-card" style={{ gridColumn: 'span 12' }}>
                ...
                <div className="admin-card-body">
                  ...
                  <hr style={{ border: 0, borderTop: '1px solid #e5e7eb', margin: '1.25rem 0' }} />
                  <h3 className="admin-card-title" style={{ fontSize: '1.05rem', marginBottom: '0.5rem' }}>
                    Partner portal (/business)
                  </h3>
                  <p className="admin-form-hint" style={{ marginTop: 0 }}>
                    Control the venue dashboard partners see (separate from the staff admin console). When the portal is off,
                    /business shows a short notice instead.
                  </p>
                  <div className="admin-form-group">
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.enabled !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: { ...(f.businessPortal || {}), enabled: e.target.checked },
                          }))
                        }
                      />
                  ...
                  <p className="admin-form-hint" style={{ fontWeight: 600, marginBottom: '0.35rem' }}>
                    Dashboard copy (optional overrides)
                  </p>
                  <div className="admin-form-row">
                    <div className="admin-form-group">
                      <label htmlFor="bp-kicker">Kicker line</label>
                      <input
                        id="bp-kicker"
                        value={form?.businessPortal?.copy?.dashboardKicker || ''}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              copy: { ...(f.businessPortal?.copy || {}), dashboardKicker: e.target.value },
                            },
                          }))
                        }
                        placeholder="e.g. Your venue command center"
                      />
                    </div>
                    <div className="admin-form-group">
                      ...
                      <input
                        id="bp-title"
                        value={form?.businessPortal?.copy?.dashboardTitle || ''}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              copy: { ...(f.businessPortal?.copy || {}), dashboardTitle: e.target.value },
                            },
                          }))
                        }
                        placeholder="e.g. Run your Tripoli listing"
                      />
                    </div>
                  </div>
                  <div className="admin-form-group">
                    <label htmlFor="bp-lead">Intro paragraph</label>
                    <textarea
                      id="bp-lead"
                      rows={2}
                      value={form?.businessPortal?.copy?.dashboardLead || ''}
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          businessPortal: {
                            ...(f.businessPortal || {}),
                            copy: { ...(f.businessPortal?.copy || {}), dashboardLead: e.target.value },
                          },
                        }))
                      }
                      placeholder="Short welcome for partners (optional)"
                    />
                  </div>
                  <p className="admin-form-hint" style={{ fontWeight: 600, marginBottom: '0.35rem' }}>
                    Partner features
                  </p>
                  <div className="admin-form-group" style={{ display: 'grid', gap: '0.5rem' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.feed !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), feed: e.target.checked },
                            },
                          }))
                        }
                      />
                      Community feed composer (posts & reels)
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.sponsorship !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), sponsorship: e.target.checked },
                            },
                          }))
                        }
                      />
                      Sponsorship page (requires checkout enabled above)
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.listingReviews !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), listingReviews: e.target.checked },
                            },
                          }))
                        }
                      />
                      Reviews tab on listing editor
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.listingTranslations !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), listingTranslations: e.target.checked },
                            },
                          }))
                        }
                      />
                      Languages tab (AR / FR overrides)
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.listingMapPicker !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), listingMapPicker: e.target.checked },
                            },
                          }))
                        }
                      />
                      Map picker on listing location
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.listingAdvancedImageUrls === true}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: {
                                ...(f.businessPortal?.sections || {}),
                                listingAdvancedImageUrls: e.target.checked,
                              },
                            },
                          }))
                        }
                      />
                      Show “image URLs” field on Photos tab (default: upload / drag / paste only)
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={form?.businessPortal?.sections?.listingDiningExtras !== false}
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            businessPortal: {
                              ...(f.businessPortal || {}),
                              sections: { ...(f.businessPortal?.sections || {}), listingDiningExtras: e.target.checked },
                            },
                          }))
                        }
                      />
                      Restaurant links block on listing (food categories)
                    </label>
                  </div>
                  <div className="admin-form-group">
